...
import { 
  ...
  FiImage, FiUpload, FiAlertCircle, FiFolder, FiPlus, FiTag, FiStar, FiUsers
} from 'react-icons/fi'
...
export const ProjectEditor = ({ project, onClose }) => {
  ...
  const handleUpload = async (file, fileType, customName = null, replace = false) => {
    const result = await uploadFile(project.id, file, fileType, customName, replace)
    if (result.success) {
  ...
  const handleSaveSettings = async () => {
    if (projectSettings.is_published && projectScenes.length === 0) {
      addToast('Нельзя опубликовать проект без сцен. Добавьте хотя бы одну сцену.', 'warning')
  ...
  const coverPreviewUrl = projectSettings.cover_url
    ? (projectSettings.cover_url.startsWith('http') ? projectSettings.cover_url : `http://localhost:8000${projectSettings.cover_url}`)
    : null
    ...
    <div className="project-editor">
      <div className="editor-header">
        ...
        <div className="header-right">
          {coverPreviewUrl && <img src={coverPreviewUrl} alt="cover" className="project-cover-thumb" />}
          <div className="scene-counter">
            <FiFolder /> {projectScenes.length} {projectScenes.length === 1 ? 'сцена' : projectScenes.length >= 2 && projectScenes.length <= 4 ? 'сцены' : 'сцен'}
          </div>
          <button onClick={() => setShowSettings(true)} className="settings-btn" title="Настройки проекта"><FiSettings /></button>
          <button onClick={() => setShowPreview(true)} className="preview-btn" title="Предпросмотр"><FiEye /></button>
          <button onClick={() => setShowDeleteConfirm(true)} className="delete-project-btn" title="Удалить проект"><FiTrash2 /></button>
          <button onClick={onClose} className="close-btn" title="Закрыть"><FiX /></button>
        </div>
      </div>
      ...
      <div className="editor-content">
        <ResourceBar
          ...
          onSelectResource={(file, type) => {
            if (type === 'backgrounds' && currentScene) {
              setCurrentScene({ ...currentScene, background_url: file.url, background_type: file.type === 'video' ? 'video' : 'image' })
            }
          }}
        ...
        {!editingScene ? (
          ...
          <SceneBuilder
            ...
            onClose={() => { setEditingScene(false); setCurrentScene(null) }}
            sprites={projectFiles.sprites || []}
            music={projectFiles.music || []}
          />
        )}
      ...
      {showPreview && previewProject && (
        <ProjectPreview project={previewProject} onClose={() => setShowPreview(false)} hidePoints={false} />
      )}
        ...
        <div className="modal-overlay" onClick={() => setShowSettings(false)}>
          <div className="modal-content settings-modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button className="modal-close" onClick={() => setShowSettings(false)}><FiX /></button>
            </div>
            <div className="modal-body">
              <div className="form-group">
                <label>Название</label>
                <input type="text" value={projectSettings.title} onChange={e => setProjectSettings({ ...projectSettings, title: e.target.value })} />
              </div>
              
              <div className="form-group">
                <label>Описание</label>
                <textarea value={projectSettings.description} onChange={e => setProjectSettings({ ...projectSettings, description: e.target.value })} rows={3} />
              </div>
              
              <div className="form-group">
                <label>Обложка проекта</label>
                <div className="cover-upload-container">
                  <input type="file" accept="image/*" onChange={handleCoverUpload} className="cover-file-input" id="cover-upload" />
                  <label htmlFor="cover-upload" className="cover-upload-label">
                    ...
                    <span className="cover-upload-hint">PNG, JPG, GIF, WEBP</span>
                  </label>
                  {coverPreviewUrl && (
                    <div className="cover-preview-container">
                      <img src={coverPreviewUrl} alt="Preview" className="cover-preview-image" />
                      <button className="cover-preview-remove" onClick={() => setProjectSettings({ ...projectSettings, cover_url: '' })} title="Удалить обложку"><FiX /></button>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="form-group">
                <label>Минимальные баллы для статуса</label>
                <input type="number" value={projectSettings.min_points} onChange={e => setProjectSettings({ ...projectSettings, min_points: parseInt(e.target.value) || 0 })} min="0" />
              </div>

              <div className="form-group">
                <label>Статус за прохождение</label>
                <div className="reward-status-container">
                  <select value={projectSettings.reward_status} onChange={e => setProjectSettings({ ...projectSettings, reward_status: e.target.value })} className="reward-status-select">
                    {allStatuses.map(status => (
                      <option key={status.id} value={status.name}>{status.name}</option>
                    ))}
                  </select>
                  <div className="new-reward-status-input-group">
                    <input type="text" value={showNewRewardStatusInput} onChange={(e) => setShowNewRewardStatusInput(e.target.value)} placeholder="Название нового статуса" className="new-status-input" />
                    <button onClick={handleAddNewRewardStatus} className="add-reward-status-btn" disabled={addingRewardStatus || !showNewRewardStatusInput.trim()}>
                      {addingRewardStatus ? '...' : 'Создать'}
                    </button>
                  </div>
                </div>
                <small className="hint">Студент получит этот статус после прохождения</small>
              </div>
                ...
                <div className="required-statuses-container">
                  <div className="selected-statuses">
                    {projectSettings.required_statuses.map(status => (
                      <div key={status} className="status-tag selected">
                        {status}
                        <button onClick={() => setProjectSettings({ ...projectSettings, required_statuses: projectSettings.required_statuses.filter(s => s !== status) })} className="remove-status">×</button>
                      </div>
                    ))}
                  ...
                  <div className="statuses-list">
                    ...
                    <div className="status-options">
                      {allStatuses.map(status => (
                        <div key={status.id} className={`status-option ${projectSettings.required_statuses?.includes(status.name) ? 'selected' : ''}`}
                          onClick={() => {
                            if (projectSettings.required_statuses?.includes(status.name)) {
                              setProjectSettings({ ...projectSettings, required_statuses: projectSettings.required_statuses.filter(s => s !== status.name) })
                            } else {
                              setProjectSettings({ ...projectSettings, required_statuses: [...(projectSettings.required_statuses || []), status.name] })
                            }
                          }}
                        >{status.name}</div>
                      ))}
                    ...
                    {projectSettings.group_ids && projectSettings.group_ids.map(groupId => {
                      ...
                      return group ? (
                        <div key={groupId} className="status-tag selected">
                          {group.name}
                          <button onClick={() => setProjectSettings({ ...projectSettings, group_ids: projectSettings.group_ids.filter(id => id !== groupId) })} className="remove-status">×</button>
                        </div>
                      ) : null
                    })}
                  ...
                  <div className="statuses-list">
                    ...
                    <div className="status-options">
                      {groupsList.map(group => (
                        <div key={group.id} className={`status-option ${projectSettings.group_ids?.includes(group.id) ? 'selected' : ''}`}
                          onClick={() => {
                            if (projectSettings.group_ids?.includes(group.id)) {
                              setProjectSettings({ ...projectSettings, group_ids: projectSettings.group_ids.filter(id => id !== group.id) })
                            } else {
                              setProjectSettings({ ...projectSettings, group_ids: [...(projectSettings.group_ids || []), group.id] })
                            }
                          }}
                        >{group.name}</div>
                      ))}
              ...
              <div className="form-group checkbox-group">
                <label htmlFor="publish-checkbox" className="checkbox-label"><FiEye /> Опубликовать проект</label>
                <input type="checkbox" id="publish-checkbox" checked={projectSettings.is_published}
                  onChange={(e) => { e.stopPropagation(); setProjectSettings({ ...projectSettings, is_published: e.target.checked }) }}
                  className="checkbox-input"
                />
              </div>
            </div>
            <div className="modal-footer">
              <button onClick={handleSaveSettings} className="save-btn"><FiSave /> Сохранить</button>
              <button onClick={() => setShowSettings(false)} className="cancel-btn"><FiX /> Отмена</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="modal-overlay" onClick={() => setShowDeleteConfirm(false)}>
          <div className="confirm-dialog" onClick={e => e.stopPropagation()}>
            ...
            <div className="confirm-actions">
              ...
              <button onClick={() => setShowDeleteConfirm(false)} className="confirm-no"><FiX /> Отмена</button>
            </div>
          </div>
        </div>
      )}
